    ...
    <>
      <section className="bg-gradient-to-br from-isa-dark to-isa-pink-700 text-white pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-isa-pink-100 text-lg max-w-xl">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="space-y-6">
              ...
              {[
                { icon: <Mail className="w-5 h-5 text-isa-pink-500" />, label: "E-mail", value: "contato@isasaude.com" },
                { icon: <Phone className="w-5 h-5 text-isa-pink-500" />, label: "Telefone", value: "(11) 0000-0000" },
                { icon: <MapPin className="w-5 h-5 text-isa-pink-500" />, label: "Localização", value: "São Paulo, SP — Brasil" },
              ].map((item) => (
                <div key={item.label} className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-isa-pink-50 rounded-lg flex items-center justify-center flex-shrink-0">
                    {item.icon}
                  </div>
                  <div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input required type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                          className="w-full border border-isa-gray-200 rounded-button px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isa-pink-500" placeholder="Seu nome completo" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-isa-gray-700 mb-1.5">E-mail *</label>
                        <input required type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                          className="w-full border border-isa-gray-200 rounded-button px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isa-pink-500" placeholder="seu@email.com" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                          className="w-full border border-isa-gray-200 rounded-button px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isa-pink-500" placeholder="(11) 99999-9999" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-isa-gray-700 mb-1.5">Assunto</label>
                        <select value={form.subject} onChange={e => setForm(p => ({ ...p, subject: e.target.value }))}
                          className="w-full border border-isa-gray-200 rounded-button px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isa-pink-500 bg-white">
                    ...
                    <div>
                      ...
                      <textarea required rows={5} value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))}
                        className="w-full border border-isa-gray-200 rounded-button px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isa-pink-500 resize-none"
